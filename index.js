...
bot.command("removeallbot", async (ctx) => {
    ...
    if (!OWNER_ID(ctx.from.id) && !isOwner(ctx.from.id)) {
        return await ctx.reply("❌ BOT OWNER MOTU PATLU HAI BHAI USE CONTECT KARO AB TISION NA KARO NUMBER ADD KAR DIYA HU SO BOT BUY KARO OR PELO NUMBER JO BANDA BUY KARI GA USE 24X7 BOT ONLINE RHI GA BUY ONLY 200RS.");
    }
...
╰═════════════════════⊱`;
        ...
        await ctx.reply("❌ BOT OWNER MOTU PATLU HAI BHAI USE CONTECT KARO PRICE 200RS HAI session.");
    }
});
...
╰═════════════════════⊱`;
        ...
        await ctx.reply("❌ BOT ACCES BUY KARO ONLY 200RS session.");
    }
});
...
bot.command("addpairing", async (ctx) => {
    // Permission check
    if (!OWNER_ID(ctx.from.id) && !isOwner(ctx.from.id)) {
        return await ctx.reply("❌ PERMISION, TO LELE MOTU PATLU SE BUY ONLY 200RS.");
    }
...
╰═════════════════════⊱`;
    ...
    try {
        if (!zephy || !isWhatsAppConnected) {
            await ctx.reply("⏳ MOTU PATLU V2.8 WhatsApp...");
            await startSesi();
...
bot.command("addowner", async (ctx) => {
    if (!OWNER_ID(ctx.from.id)) {
        return await ctx.reply("❌ PERMISION, TO LELE MOTU PATLU SE only 200rs.");
    }
...
const checkWhatsAppConnection = async (ctx, next) => {
    if (!isWhatsAppConnected) {
        await ctx.reply("❌ WhatsApp connect karlo bhai. script by motu PATLU command /addpairing");
        return;
    }
    await next();
};
...
bot.start(async (ctx) => {
   ...
   *© 2025 𝗠𝗢𝗧𝗨 𝗣𝗔𝗧𝗟𝗨 𝗩𝟮.𝟴 Crasher Beta Version - @MOTU_PATALU_HINDU_HAI*
   `;
    ...
    setTimeout(async () => {
        await ctx.replyWithVideo("https://files.catbox.moe/6yad02.mp4", {
...
𝙈𝙊𝙏𝙐𝙋𝘼𝙏𝙇𝙐 𝘾𝙍𝘼𝙎𝙃𝙀𝙍 | @Hx54x56`;
    ...
    await ctx.replyWithVideo("https://files.catbox.moe/8tjnrw.mp4", {
        caption: mainMenuMessage,
        parse_mode: "Markdown",
        reply_markup: {
            inline_keyboard: mainKeyboard
...
𝙈𝙊𝙏𝙐𝙋𝘼𝙏𝙐𝙇𝙐 𝘾𝙍𝘼𝙎𝙃𝙀𝙍 | @BUG_MOTU_PATLU`;
    ...
    await ctx.replyWithPhoto("https://files.catbox.moe/whlobn.jpg", {
        caption: mainMenuMessage,
        parse_mode: "Markdown",
        reply_markup: {
            inline_keyboard: mainKeyboard
...
bot.action('bugmenu', async (ctx) => {
  ...
  *© 2025 𝗠𝗼𝘁𝘂 𝗽𝗮𝘁𝗹𝘂 𝙑2.𝟕*`;
    ...
    try {
        await ctx.deleteMessage();
        await ctx.replyWithVideo("https://files.catbox.moe/1qaze2.mp4", {
            caption: mainMenuMessage,
            parse_mode: "Markdown",
            reply_markup: {
                inline_keyboard: mainKeyboard
        ...
        viewOnceMessage: {
            message: {
                listResponseMessage: {
                    ...
                    fileName: "Sasuke Crash☠️",
                    fileEncSha256: "wAzguXhFkO0y1XQQhFUI0FJhmT8q7EDwPggNb89u+e4=",
                    directPath: "/text/t62.7119-24/23916836_520634057154756_7085001491915554233_n.enc?ccb=11-4&oh=01_Q5AaIC-Lp-dxAvSMzTrKM5ayF-t_146syNXClZWl3LMMaBvO&oe=66F0EDE2&_nc_sid=5e03e0",
